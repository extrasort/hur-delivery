-- Add bulk_order_id reference to orders table to track bulk orders
ALTER TABLE orders 
ADD COLUMN IF NOT EXISTS bulk_order_id UUID REFERENCES bulk_orders(id) ON DELETE SET NULL;

-- Add index for bulk order queries
CREATE INDEX IF NOT EXISTS idx_orders_bulk_order_id 
  ON orders(bulk_order_id) 
  WHERE bulk_order_id IS NOT NULL;

-- Update post_bulk_order function to set bulk_order_id
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Create individual order with bulk_order_id reference
      INSERT INTO orders (
        merchant_id,
        customer_name,
        customer_phone,
        ...
        COALESCE(bulk_record.vehicle_type, 'motorcycle'),
      ...
      RAISE NOTICE 'Created order: % (bulk_order_id: %)', new_order_id, p_bulk_order_id;
...
COMMENT ON COLUMN orders.bulk_order_id IS 'Reference to bulk order if this order is part of a bulk order group';

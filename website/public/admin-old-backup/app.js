...
const messagingFilterState = {
    search: '',
    role: 'all',
};
let messagingSelectedUser = null;
let messagingUserResults = [];
let messagingUserSearchTimer = null;
...
function initializeSupabase() {
    try {
        ...
        console.log('✅ Supabase initialized', {
            ...
            functionsUrl: `${CONFIG.SUPABASE_URL}/functions/v1`,
        });
    } catch (error) {
...
function setupEventListeners() {
    ...
    document.getElementById('messagingMessages')?.addEventListener('click', handleMessagingMessageAction);
    document.getElementById('messagingSearch')?.addEventListener('input', handleMessagingSearch);
    document.getElementById('messagingRoleFilter')?.addEventListener('change', handleMessagingRoleFilter);
    document.getElementById('messagingNewConversationBtn')?.addEventListener('click', openMessagingNewConversationModal);
    document.getElementById('messagingReplyCancel')?.addEventListener('click', cancelMessagingReply);
    document.getElementById('messagingNewConversationForm')?.addEventListener('submit', handleMessagingCreateConversation);
    document.getElementById('messagingUserSearch')?.addEventListener('input', handleMessagingUserSearchInput);
    document.getElementById('messagingUserResults')?.addEventListener('click', handleMessagingUserResultClick);
    document.getElementById('messagingNewConversationCancel')?.addEventListener('click', closeMessagingNewConversationModal);
}
    ...
    if (storedSession) {
        try {
            ...
            if (session?.user?.role === 'admin') {
                // Restore Supabase session for authenticated RPCs
                if (session.supabase_session?.access_token && session.supabase_session?.refresh_token) {
                    try {
                        ...
                        console.warn('Failed to restore admin auth session:', e?.message || e);
                    }
                }
...
async function handleLogin(e) {
    ...
    try {
        // debug logs
        console.log('[admin-login] invoking edge function');
        const invokePromise = supabaseClient.functions.invoke('admin-login', {
            body: { username, password },
        });
        let data, error;
        try {
            ({ data, error } = await withTimeout(invokePromise, 30000, 'طلب تسجيل الدخول'));
        } catch (timeoutErr) {
            // Fallback to direct fetch if invoke stalls
            try {
                const res = await withTimeout(
                    fetch(`${CONFIG.SUPABASE_URL}/functions/v1/admin-login`, {
                        method: 'POST',
                        headers: {
                ...
                throw timeoutErr; // surface original timeout
            }
        }
        ...
        try {
            ...
            console.warn('Upsert admin user failed (non-fatal):', e?.message || e);
        }
        const adminSession = {
            user: {
        ...
        errorDiv.textContent = err.message || 'فشل تسجيل الدخول / Login failed';
        errorDiv.classList.add('active');
    } finally {
        if (submitBtn) {
...
async function loadMessaging() {
    ...
    if (typeof AdminMessaging === 'undefined') {
        console.warn('AdminMessaging module not loaded');
        return;
    }
...
function ensureMessagingElements() {
    ...
    messagingElements.search = document.getElementById('messagingSearch');
    messagingElements.roleFilter = document.getElementById('messagingRoleFilter');
    messagingElements.newConversationBtn = document.getElementById('messagingNewConversationBtn');
    messagingElements.replyPreview = document.getElementById('messagingReplyPreview');
    messagingElements.replyPreviewAuthor = document.getElementById('messagingReplyAuthor');
    messagingElements.replyPreviewSnippet = document.getElementById('messagingReplySnippet');
    messagingElements.userResults = document.getElementById('messagingUserResults');
    messagingElements.orders = document.getElementById('messagingOrders');
}
...
function renderMessagingConversations(conversations = AdminMessaging?.state?.conversations || []) {
    ...
    const filtered = applyMessagingFilters(conversations);
    const container = messagingElements.conversations;
    container.innerHTML = '';

    if (messagingElements.search && messagingElements.search !== document.activeElement) {
        messagingElements.search.value = messagingFilterState.search;
    }
    if (messagingElements.roleFilter && messagingElements.roleFilter !== document.activeElement) {
        messagingElements.roleFilter.value = messagingFilterState.role;
    }
    ...
    filtered.forEach((conversation) => {
        ...
        item.dataset.counterpartRole = (meta.counterpart?.role || '').toLowerCase();
        item.dataset.orderId = conversation.order_id || '';

        const lastTime = formatMessagingTimestamp(meta.lastMessageAt || conversation.created_at);
        const preview = meta.lastMessagePreview || 'لا توجد رسائل بعد';
        ...
        item.innerHTML = `
            ...
            <p class="messaging-item-subtitle">${escapeHtml(subtitle)}</p>
            <p class="messaging-item-preview">${escapeHtml(preview)}</p>
        `;
    ...
    const active = AdminMessaging.state.activeConversationId
        ? filtered.find(c => c.id === AdminMessaging.state.activeConversationId) ||
          conversations.find(c => c.id === AdminMessaging.state.activeConversationId)
        : null;
...
function renderMessagingMessages(messages = AdminMessaging?.state?.messages || []) {
    ...
    messages.forEach((message) => {
        ...
        if (message.meta?.isMine) div.classList.add('admin');
        if (AdminMessaging.state.replyingTo?.id === message.id) div.classList.add('reply-target');

        const quote = message.meta?.replyPreview
            ? `<div class="message-quote"><strong>${escapeHtml(message.meta.replyPreview.author || 'مستخدم')}</strong><p>${escapeHtml(truncateText(message.meta.replyPreview.text || '', 160))}</p></div>`
            : '';
        const orderTag = message.order_id ? `<span class="message-order">طلب: ${escapeHtml(message.order_id)}</span>` : '';

        div.innerHTML = `
            ${quote}
            <div class="message-body">${escapeHtml(message.body || '')}${orderTag}</div>
            <div class="message-meta">
                <span class="message-sender">${escapeHtml(message.meta?.displayName || '')}</span>
                <time>${escapeHtml(message.meta?.createdLabel || '')}</time>
                <button class="message-action message-reply" data-message-id="${message.id}" title="رد">
...
function renderMessagingReplyPreview() {
    ensureMessagingElements();
    if (!messagingElements.replyPreview) return;

    const replyingTo = AdminMessaging?.state?.replyingTo || null;
    if (!replyingTo) {
        messagingElements.replyPreview.classList.add('hidden');
        if (messagingElements.replyPreviewAuthor) messagingElements.replyPreviewAuthor.textContent = '';
        if (messagingElements.replyPreviewSnippet) messagingElements.replyPreviewSnippet.textContent = '';
        return;
    }

    messagingElements.replyPreview.classList.remove('hidden');
    if (messagingElements.replyPreviewAuthor) {
        messagingElements.replyPreviewAuthor.textContent = replyingTo.meta?.displayName || 'مستخدم';
    }
    if (messagingElements.replyPreviewSnippet) {
        messagingElements.replyPreviewSnippet.textContent = truncateText(replyingTo.body || '', 160);
    }
}

function applyMessagingFilters(conversations) {
    const term = (messagingFilterState.search || '').toLowerCase();
    const role = (messagingFilterState.role || 'all').toLowerCase();

    return conversations.filter((conversation) => {
        const meta = conversation.meta || {};
        if (role !== 'all') {
            const counterpartRole = (meta.counterpart?.role || '').toLowerCase();
            if (counterpartRole !== role) return false;
        }
        if (!term) return true;
        const haystack = [
            meta.title,
            meta.subtitle,
            meta.counterpart?.name,
            meta.counterpart?.phone,
...
async function handleMessagingConversationClick(event) {
    ...
    AdminMessaging.setReplyTo(null);
...
function handleMessagingMessageAction(event) {
    const replyBtn = event.target.closest('.message-reply');
    if (replyBtn && typeof AdminMessaging !== 'undefined') {
        const messageId = replyBtn.dataset.messageId;
        if (messageId) {
            AdminMessaging.setReplyTo(messageId);
            renderMessagingReplyPreview();
        }
    }
}
...
function cancelMessagingReply() {
    if (typeof AdminMessaging === 'undefined') return;
    AdminMessaging.clearReplyTo();
    renderMessagingReplyPreview();
}

function handleMessagingSearch(event) {
    messagingFilterState.search = event.target.value;
    renderMessagingConversations();
}

function handleMessagingRoleFilter(event) {
    messagingFilterState.role = event.target.value || 'all';
    renderMessagingConversations();
}
...
function setActiveConversationUI(conversation) {
    ...
    const meta = conversation.meta || {};
    messagingElements.title.textContent = meta.title || 'محادثة';

    const subtitleParts = [];
    if (meta.counterpartRole) subtitleParts.push(meta.counterpartRole);
    if (meta.counterpart?.phone) subtitleParts.push(meta.counterpart.phone);
    if (conversation.order_id) subtitleParts.push(`رقم الطلب: ${conversation.order_id}`);
    if (!subtitleParts.length && meta.subtitle) subtitleParts.push(meta.subtitle);

    if (messagingElements.subtitle) {
        messagingElements.subtitle.textContent = subtitleParts.join(' • ');
    }
    renderMessagingReplyPreview();
}
...
function renderMessagingUserResults(message) {
    const list = document.getElementById('messagingUserResults');
    if (!list) return;
    ...
    if (!messagingUserResults.length) {
        list.innerHTML = '<p class="empty-state">ابحث عن مستخدم لبدء المحادثة</p>';
        return;
    }

    list.innerHTML = messagingUserResults
        .map((user) => {
            const isSelected = messagingSelectedUser?.id === user.id;
            const roleLabel = formatUserRoleLabel(user.role);
            const name = user.name || user.phone || user.email || 'مستخدم';
            const secondary = user.phone || user.email || '';
            return `
                <div class="messaging-user-result ${isSelected ? 'selected' : ''}" data-user-id="${user.id}">
...
function openMessagingNewConversationModal() {
    messagingSelectedUser = null;
    messagingUserResults = [];
    const searchInput = document.getElementById('messagingUserSearch');
    const orderInput = document.getElementById('messagingOrderInput');
    const initialMessageInput = document.getElementById('messagingInitialMessage');
    if (searchInput) searchInput.value = '';
    if (orderInput) orderInput.value = '';
    if (initialMessageInput) initialMessageInput.value = '';
    renderMessagingUserResults();
    openModal('messagingNewConversationModal');
    searchInput?.focus();
}

function closeMessagingNewConversationModal() {
    document.getElementById('messagingNewConversationModal')?.classList.remove('active');
}

function handleMessagingUserSearchInput(event) {
    const term = event.target.value.trim();
    messagingSelectedUser = null;
    if (messagingUserSearchTimer) clearTimeout(messagingUserSearchTimer);
    if (!term) {
        messagingUserResults = [];
        renderMessagingUserResults();
        return;
    }
    messagingUserSearchTimer = setTimeout(() => searchMessagingUsers(term), 350);
}

function handleMessagingUserResultClick(event) {
    const item = event.target.closest('.messaging-user-result');
    if (!item) return;
    const userId = item.dataset.userId;
    messagingSelectedUser = messagingUserResults.find((user) => user.id === userId) || null;
    renderMessagingUserResults();
}

async function handleMessagingCreateConversation(event) {
    event.preventDefault();
    if (typeof AdminMessaging === 'undefined') return;
    if (!messagingSelectedUser) {
        showError('يرجى اختيار مستخدم لبدء المحادثة');
        return;
    }

    const orderId = document.getElementById('messagingOrderInput')?.value.trim() || null;
    const initialMessage = document.getElementById('messagingInitialMessage')?.value.trim() || null;

    try {
        await AdminMessaging.startSupportConversation({
            userId: messagingSelectedUser.id,
            orderId: orderId || null,
            initialMessage: initialMessage || undefined,
        });
        closeMessagingNewConversationModal();
        showSuccess('تم إنشاء محادثة جديدة بنجاح');
        renderMessagingConversations();
        renderMessagingMessages();
        messagingElements.compose?.focus();
    } catch (error) {
        console.error('Failed to create conversation', error);
        showError('تعذر إنشاء محادثة جديدة');
    }
}

async function searchMessagingUsers(term) {
    ...
    try {
        ...
        const { data, error } = await supabaseClient
            ...
            .limit(20);
        if (error) throw error;
        messagingUserResults = data || [];
        renderMessagingUserResults();
    } catch (error) {
        console.error('Failed to search users', error);
        renderMessagingUserResults('<p class="error">تعذر البحث عن المستخدمين</p>');
    }
}
...
function resetMessagingState() {
    ...
    messagingFilterState.search = '';
    messagingFilterState.role = 'all';
    messagingSelectedUser = null;
    messagingUserResults = [];
    if (typeof AdminMessaging !== 'undefined') {
        AdminMessaging.destroy?.();
        AdminMessaging.state.conversations = [];
        AdminMessaging.state.messages = [];
        AdminMessaging.state.activeConversationId = null;
        AdminMessaging.state.replyingTo = null;
    }
    ensureMessagingElements();
    ...
    if (messagingElements.search) messagingElements.search.value = '';
    if (messagingElements.roleFilter) messagingElements.roleFilter.value = 'all';
    renderMessagingReplyPreview();
    renderMessagingConversations();
    renderMessagingMessages();
    renderMessagingUserResults();
...
async function withTimeout(promise, ms, label = 'operation') {
    ...
    const timeoutPromise = new Promise((_, reject) => {
        timeoutId = setTimeout(() => reject(new Error(`انتهت مهلة ${label} / ${label} timed out`)), ms);
    });
    try {
...
function renderMessagingOrders(orders = []) {
    ensureMessagingElements();
    const wrapper = messagingElements.orders;
    if (!wrapper) return;
    const content = wrapper.querySelector('.panel-content');
    if (!content) return;

    if (!orders.length) {
        content.innerHTML = '<div class="empty-state">لا توجد طلبات / No orders</div>';
        return;
    }

    content.innerHTML = orders
        .map((o) => {
            const created = formatMessagingTimestamp(o.created_at);
            return `
            <div class="order-item" data-order-id="${o.id}">
                ...
                <div class="order-actions">
                    <select class="order-status-select" data-action="status" data-order-id="${o.id}">
                        ${['pending','assigned','accepted','on_the_way','delivered','cancelled','rejected']
                            .map(s => `<option value="${s}" ${s===o.status?'selected':''}>${s}</option>`).join('')}
                    </select>
                    ${o.customer_phone ? `<a class="btn-icon" data-action="call" data-phone="${o.customer_phone}" title="اتصال"><i class="fas fa-phone"></i></a>` : ''}
                    <button class="btn-icon" data-action="reassign" data-order-id="${o.id}" title="إعادة تعيين"><i class="fas fa-random"></i></button>
                </div>
            </div>
            `;
        })
...
// Click handlers for order actions
if (document.getElementById('messagingOrders')) {
    document.getElementById('messagingOrders').addEventListener('change', async (e) => {
        const target = e.target;
        if (target && target.matches('.order-status-select')) {
    ...
    document.getElementById('messagingOrders').addEventListener('click', async (e) => {
        ...
        if (action === 'reassign') {
            ...
            try {
                await AdminMessaging.quickReassign?.(orderId);
                showSuccess('تم إعادة تعيين الطلب');
            } catch (err) {
                console.error('Failed to reassign', err);
                showError('تعذر إعادة تعيين الطلب');
            }
        } else if (action === 'call') {
...
// Subscribe to orders updates from AdminMessaging
if (typeof AdminMessaging !== 'undefined') {
    AdminMessaging.onOrders(renderMessagingOrders, { immediate: false });
}
